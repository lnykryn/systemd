...
static int get_sysattr_value (sd_device *device, const char *sysattr, const char **ret_value)  {
        int r;
        const char *defined = NULL;
        const char *version = NULL;
        const char *introduced = NULL;
        char *key = NULL;
        log_info("1. Reading '%s'.", sysattr);

        r = sd_device_get_property_value(device, "SYSFS_ATTRIBUTES_DEFINED", &defined);
        if (r < 0 || !streq(defined, "1"))
                goto READ_ATTR;

        log_info("2. Defined '%s'.", defined);

        r = sd_device_get_property_value(device, "SYSATTR_VERSION", &version);
        if (r < 0)
                goto READ_ATTR;

        log_info("3. Version '%s'.", version);

        key = strjoina("SYSFS_ATTRIBUTE_", sysattr);

        log_info("4. Key '%s'.", key);

        r = sd_device_get_property_value(device, key, &introduced);
        if (r < 0 )
                return r;

        log_info("5. Introduced '%s'.", introduced);

        log_info("6. VERCMP '%d'.", strverscmp_improved(introduced, version));


        if (strverscmp_improved(introduced, version) > 0)
                return -ENOENT;

READ_ATTR:

        r = sd_device_get_sysattr_value(device,sysattr, ret_value);
...
static int dev_pci_onboard(sd_device *dev, const LinkInfo *info, NetNames *names) {
        ...
        if (get_sysattr_value(names->pcidev, "acpi_index", &attr) >= 0)
                log_device_debug(names->pcidev, "acpi_index=%s", attr);
        else {
                /* SMBIOS type 41 — Onboard Devices Extended Information */
                r = get_sysattr_value(names->pcidev, "index", &attr);
                if (r < 0)
                        return r;
        ...
        if (get_sysattr_value(dev, "dev_port", &attr) >= 0) {
        ...
        if (get_sysattr_value(names->pcidev, "label", &names->pci_onboard_label) >= 0)
...
static bool is_pci_ari_enabled(sd_device *dev) {
        ...
        if (get_sysattr_value(dev, "ari_enabled", &a) < 0)
                return false;
...
static bool is_pci_bridge(sd_device *dev) {
        ...
        if (get_sysattr_value(dev, "modalias", &v) < 0)
                return false;
...
static int parse_hotplug_slot_from_function_id(sd_device *dev, int slots_dirfd, uint32_t *ret) {
        ...
        if (get_sysattr_value(dev, "function_id", &attr) < 0)
                return 0;
...
static int dev_pci_slot(sd_device *dev, const LinkInfo *info, NetNames *names) {
        ...
        if (get_sysattr_value(dev, "dev_port", &attr) >= 0) {
                ...
                if (dev_port == 0 &&
                    ...
                    get_sysattr_value(dev, "dev_id", &attr) >= 0) {
        ...
        while (hotplug_slot_dev) {
                ...
                FOREACH_DIRENT_ALL(de, dir, break) {
                        ...
                        if (get_sysattr_value(pci, path, &address) < 0)
                                continue;
...
static int dev_devicetree_onboard(sd_device *dev, NetNames *names) {
        ...
        FOREACH_DEVICE_SYSATTR(aliases_dev, alias) {
                ...
                if (get_sysattr_value(aliases_dev, alias, &alias_path) < 0)
                        continue;
                ...
                if (i == 0 && get_sysattr_value(aliases_dev, conflict, NULL) >= 0)
...
static int names_mac(sd_device *dev, const LinkInfo *info) {
        ...
        r = get_sysattr_value(dev, "addr_assign_type", &s);
        if (r < 0)
...
static int get_link_info(sd_device *dev, LinkInfo *info) {
        ...
        (void) get_sysattr_value(dev, "phys_port_name", &info->phys_port_name);

        r = get_sysattr_value(dev, "address", &s);
        if (r < 0 && r != -ENOENT)
                return r;
        if (r >= 0) {
